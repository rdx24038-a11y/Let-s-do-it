...
def send_telegram(msg):
    try:
        import requests
        requests.get(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage?chat_id={TELEGRAM_CHAT_ID}&text={msg}", timeout=5)
    except:
        pass
        ...
        while True:
            try:
                ...
                for update in r.json().get("result", []):
                    ...
                    if msg == "/count":
                        requests.get(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage?chat_id={cid}&text=Total accounts: {account_counter}", timeout=5)
                    elif msg == "/download":
                        import glob
                        files = glob.glob("/app/BIGBULL-ERA/ACCOUNTS/*.txt") + glob.glob("/app/BIGBULL-ERA/ACCOUNTS/*.json")
                        if files:
                            for fp in files:
...
def generate_accounts_flow():
    ...
    # Auto send files to Telegram
    try:
        import glob, os
        send_telegram(f"✅ Cycle complete! {SUCCESS_COUNTER} accounts generated!")
        files = glob.glob(str(ACCOUNTS_FOLDER) + "/*.txt") + glob.glob(str(ACCOUNTS_FOLDER) + "/*.json")
        if files:
            for fp in files:
                send_file_telegram(fp)
        else:
            send_telegram("No account files found!")
    except Exception as e:
        pass
